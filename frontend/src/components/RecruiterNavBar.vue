...
const handleSearch = () => {
  ...
  debounceTimer = setTimeout(async () => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/api/recruiter/global_search?q=${searchQuery.value}`, {
...
<template>
  <nav class="navbar navbar-expand-lg navbar-dark bg-transparent px-4 py-3">
    <div class="container-fluid">
      <router-link class="navbar-brand fw-bold d-flex align-items-center gap-2" to="/recruiter-dashboard">
        <span class="bg-white text-primary rounded px-2 py-0 fw-bolder">CPMS</span>
        PlacementCell <span class="badge bg-white text-primary ms-2 small" style="font-size: 0.7rem;">RECRUITER</span>
      </router-link>

      <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#recNav">
        <span class="navbar-toggler-icon"></span>
      </button>

      <div class="collapse navbar-collapse" id="recNav">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0 ms-4 gap-3">
          <li class="nav-item">
            <router-link class="nav-link fw-bold" to="/recruiter-dashboard" active-class="active">Dashboard</router-link>
          </li>
          <li class="nav-item">
            <router-link class="nav-link" to="/manage-drives" active-class="fw-bold active">Manage Drives</router-link>
          </li>
          <li class="nav-item">
            <router-link class="nav-link" to="/drive-applications" active-class="fw-bold active">Applications</router-link>
          </li>
        </ul>
        ...
        <div class="d-flex align-items-center gap-3 mt-3 mt-lg-0">
          <div class="dropdown">
            <a class="nav-link dropdown-toggle text-white d-flex align-items-center gap-2" href="#" role="button" data-bs-toggle="dropdown">
              ...
              <span class="d-none d-md-block text-white small fw-bold">{{ companyName }}</span>
            </a>
