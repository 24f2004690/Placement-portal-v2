...
const handleLogin = async () => {
  ...
  try {
    ...
    // Updated CPMS Routing Logic
    if (data.role === 'admin') router.push('/admin-dashboard')
    else if (data.role === 'company') router.push('/recruiter-dashboard')
...
<style scoped>
.brand-gradient { background: linear-gradient(70deg, #1e3c72 0%, #2a5298 100%); }
...
.btn-primary { background-color: #0d6efd; border: none; transition: transform 0.2s ease, box-shadow 0.2s ease; }
